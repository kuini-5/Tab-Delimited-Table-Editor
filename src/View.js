...
const SaveTable = (CurrentItem, TableName, Tblidx) => {
  ...
  fs.writeFileSync(`./script${config['ServerType']}/${TableName}.txt`, output)
...
export default function View() {
  ...
  ipcRenderer.on('fromMainWindow', function (event, arg) {
    if (state.isLoaded === false)
      setState({ isLoaded: true, currentItem: LoadTable(GetColumnData(arg.TableName[1]), arg.TableName[1], arg.Tblidx), tableName: arg.TableName, tblidx: arg.Tblidx })
  });
  ...
  return (
    <div className="App bg-background overflow-hidden min-h-screen w-full text-pink font-bold overscroll-none p-0 m-0">
      ...
      <div className="tabs fixed flex w-full bg-line h-[50px] select-none items-center mt-[35px]">
        {state.tableName && <label className='ml-5 font-bold text-sm text-background'>{state.tableName[0]} {'>'} {state.tableName[1]}</label>}
        {state.currentItem && <button onClick={() => SaveTable(state.currentItem, state.tableName[1], state.tblidx)} className={`btn-primary w-[150px] ml-auto px-5 h-full font-bold bg-foreground hover:border-b-2 hover:-translate-y-1`}>Save Table</button>}
      </div>
                  ...
                  <label className='text-xl ml-auto'>{key}{data['loop'] >= 0 && config['ShowLoopIndex'] && `: ${data['loop']}`}</label>
                  {
                    data['options'] !== false && data['options'] !== undefined ?
                      ...
                      <div className='flex flex-row items-center space-x-2 w-full'>
                        ...
                        {data['related'] !== false && val !== "4294967295" && <FontAwesomeIcon onClick={() => ipcRenderer.send('view2', { TableName: [state.tableName[0], state.tableName[1], data['related']], Tblidx: val })} className='hover:bg-line p-2 rounded-lg cursor-pointer' icon={faUpRightFromSquare} />}
                      </div>
                  }
                </div>
              )
